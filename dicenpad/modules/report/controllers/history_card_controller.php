...
class History_card_controller extends Front_Controller
{
	...
	* @param int $user crew_id
	*/
	public function crew_interview($crew=null)
	{		
		$personal = $this->rest->get('crew', array('hash' => $crew), 'json');

		if(!isset($personal)) redirect('crew-master-list');

		$vessel = $this->rest->get('vessel', array('id' => _p($personal->interview, 'vessel_id')), 'json');

		$personal->vessel = isset($vessel)? $vessel : '';

		$setting = array('inPDF' => $this->report_deploy, 'data' =>  $personal, 'file_loc' => 'crew/crew_interview',
						 'paper_size' => 0, 'orientation' => 'portrait', 'name' => 'crew-interview' );
	...
	*/
	...
	//	$crews->dis_vesselname = $crews->dis_vesselname;
	...
	//	$crews->dis_vesselname = $crews->dis_vesselname;
		...
		$setting = array('inPDF' => $this->report_deploy, 'data' =>  $crews, 'file_loc' => 'crew/crew_list_1',
						 'paper_size' => 0, 'orientation' => 'portrait', 'name' => 'crew-list' );
