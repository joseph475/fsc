...
class Excel_report_controller extends Front_Controller
{
	...
	public function crew_principal($principal_id=null, $position_id=null, $status_id=null) 
	{
		...
		if($principal_id) $this->db->where_in('vw_crew_list.principal_id', $principal_id);
		if($position_id) $this->db->where_in('vw_crew_list.position_id', $position_id);
		if($status_id) $this->db->where_in('vw_crew_list.status_id', $status_id);

		$this->db->select('vw_crew_list.*, 
        ...
        $this->db->order_by('vw_crew_list.lastname');

        $obj = $this->db->get("vw_crew_list");
        ...
        if ($obj->num_rows > 0)
        {
            ...
            else
            {
		    	...
		    	$sheet->getStyle('A1:B2')->getFont()->setBold(true)->setSize('12px');
		    	$sheet->getStyle('A2:B2')->getFont()->setBold(false)->setSize('10px');
				...
				$sheet->setCellValueByColumnAndRow(0, 1, 'Crew List ' . $status);	
				$sheet->setCellValueByColumnAndRow(0, 2, 'As of ' . date('M d, Y'));

                $array = array('A'=>'7', 'B'=>'9', 'C'=>'30', 'D'=>'25', 'E'=>'12', 'F'=>'30', 'G'=>'30' ,'H'=>'50');
                ...
                $array = array(	'No', 'Crew ID', 'Name' ,'Position' , 'Birth Date', 'Vessel', 'Principal' ,'Manager\'s Remarks');
		         $col = 0;
		        foreach ($array as $field):	   
		            $sheet->setCellValueByColumnAndRow($col, 4, $field);	   
		            $col++;
		        endforeach;
	        	...
	        	foreach ($obj as $key => $value) {
					...
					$sheet->setCellValueByColumnAndRow(1, $row, $value->crew_id);
					$sheet->setCellValueByColumnAndRow(2, $row, $value->fullname);
					$sheet->setCellValueByColumnAndRow(3, $row, $value->position);
					$sheet->setCellValueByColumnAndRow(4, $row, $value->birthdate);
					$sheet->setCellValueByColumnAndRow(5, $row, $value->vessel_name . (($value->status_id == 2)? ' (Last vessel)' : ''));
					$sheet->setCellValueByColumnAndRow(6, $row, $value->principal);
					$sheet->setCellValueByColumnAndRow(7, $row, $value->manager_comment);
					$row++;	
	        	}

		        $sheet->getStyle("A4:R{$row}")->getFont()->setSize('11px')->setBold(true);
		        $sheet->getStyle("A5:R{$row}")->getFont()->setSize('10px')->setBold(false);
		        $sheet->getStyle("A5:B{$row}")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		        $sheet->getStyle("E5:E{$row}")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		        $sheet->getStyle("A5:R{$row}")->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		        $sheet->getStyle("A4:R{$row}")->getAlignment()->setWrapText(true);

				$this->excel_footer($objPHPExcel, 'crew-principal-list');
        
            }
        } 
        else
        {
           redirect('search-by-crew-principal');
        }
