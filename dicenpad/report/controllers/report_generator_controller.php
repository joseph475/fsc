...
class Report_generator_controller extends Front_Controller
{
	...
	public function generate_report3(){
		...
		$setting = array_merge((array) $setting, array('data' => $expired_doc, 'file_loc' => 'OWWA/OWWA_PAYMENT', 'name' => 'OWWA-PAYMENT' ));
		$this->pdf_footer($setting);
	}
	...
	{	
		...
		if ($setting['inPDF']) {
			...
			$size = ($setting['paper_size'] == 0)? 'landscape' :  array(0,0,950,950); //
	...
	private function tcdpf_setup($setting = array())
	{
		...
		$pdf_tc = new pdf_tc($setting['ORIENTATION'], 'landscape', $setting['PAPER_SIZE'], true, 'UTF-8', false);
